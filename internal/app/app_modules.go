...
func (a *App) provideBusinessModules(repos *repository.Set) (*businessModules, error) {
	modules := &businessModules{}

	authModule, err := (authModuleProvider{}).Provide(a, repos)
	if err != nil {
		return nil, fmt.Errorf("init auth service: %w", err)
	}
	modules.Auth = authModule

	userModule, err := (userModuleProvider{}).Provide(a, repos)
	if err != nil {
		return nil, fmt.Errorf("init user service: %w", err)
	}
	modules.User = userModule

	rbacModule, err := (rbacModuleProvider{}).Provide(a, repos)
	if err != nil {
		return nil, fmt.Errorf("init rbac service: %w", err)
	}
	modules.RBAC = rbacModule

	sampleModule, err := (sampleModuleProvider{}).Provide(a, repos)
	if err != nil {
